...
export default function TripDisplay({
  ...
  const creatorPfpUrl = creatorQuery.data.profilepicture;
    ...
    >
      {owner && (
        <div className="flex flex-col gap-2 absolute top-4 right-4 opacity-0 group-hover:opacity-100">
          ...
          <Link to={"/create/" + id}>
            <button className="text-sm font-light text-gray-500 dark:text-dark-900">Edit</button>
          </Link>
        </div>
      )}
      ...
      <div className="text-center text-xl font-semibold mt-4 dark:text-dark-900" title="TripPostTitle">
        {title}
      </div>
      <div className="py-2 px-4">
        <ol className="relative border-l border-gray-700 dark:border-white">{stopElements}</ol>
      </div>
      {description}
...
export function ListElement({ country, province, area }: Location) {
  return (
    <li className="mb-6 ml-4">
      ...
      <h3 className=" text-xs font-semibold text-gray-900 dark:text-dark-900">{country}</h3>
