...
export default function LocationDisplay({
  ...
  const leaveEditmode = (e: any, location: Location | null) => {
    const combined = location
      ? [...topLocations, location, ...bottomLocations]
      : [...topLocations, ...bottomLocations];
  ...
  return (
    <div className="p-4 rounded-xl">
      <OrderedList
        {...{ locations: topLocations, enterEditmode, deleteItem }}
      />

      {editmode ? (
        <EditMode location={editItem} goBack={leaveEditmode} />
      ) : (
        <></>
      )}
      <OrderedList
        {...{ locations: bottomLocations, enterEditmode, deleteItem }}
      />
      <button
...
function OrderedList({ locations, enterEditmode, deleteItem }: ListProps) {
  const listElements = locations
    ...
    .map((location, index) => (
      <li
        key={index}
        className="h-20 ml-4 group flex justify-between items-center"
      >
        <div>
        ...
        <div className="flex flex-row scale-0 group-hover:scale-100">
          <button onClick={(e) => enterEditmode(e, index, false)}>
            <IconEdit />
          </button>
